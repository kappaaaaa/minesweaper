...
class tile(object):
    ...
    def draw(self, win):
        ...
        if self.ishid == False and self.isflagged == False:
            ...
            pygame.draw.rect(win, (127 * (self.x - 2 * (self.x // 2) + self.y - 2 * (self.y // 2)), #creates a visible crosword-word-puzzle-like shape so we can tell where where given tiles are
                                   127 * (self.x - 2 * (self.x // 2) + self.y - 2 * (self.y // 2)),
                                   127 * (self.x - 2 * (self.x // 2) + self.y - 2 * (self.y // 2))),
                             (20 * self.x, 20 * self.y, 20, 20))

    def odkryj(self): #shows content of a given tile (number or an image of a bomb(unless its flagged, then it just removes the flag)
